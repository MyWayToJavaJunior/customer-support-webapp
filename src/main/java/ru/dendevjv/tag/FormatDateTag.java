...
public class FormatDateTag extends TagSupport {
    ...
    public int doEndTag() throws JspException {
        ...
        if (locale != null) {
            ...
            if (this.value instanceof Date)
                ...
                formatted = this.formatDate(((Calendar) this.value).getTime(),
                        locale);
            else if (this.value instanceof TemporalAccessor)
                formatted = this.formatDate((TemporalAccessor) this.value,
                        locale);
            else
    ...
    private String formatDate(Date value, Locale locale) {
        ...
        if (this.type == Type.DATE || this.type == Type.BOTH) {
            DateFormat format = DateFormat.getDateInstance(
                    this.dateStyle.getDateFormat(), locale);
            format.setTimeZone(this.timeZone);
            dateFormat = format::format;
        }
        if (this.type == Type.TIME || this.type == Type.BOTH) {
            DateFormat format = DateFormat.getTimeInstance(
                    this.timeStyle.getDateFormat(), locale);
            format.setTimeZone(this.timeZone);
    ...
    private Locale getLocale() throws JspTagException {
        try {
            return (Locale) GET_LOCALE_METHOD.invoke(null, this.pageContext,
                    this, true, true);
    ...
    private TimeZone getTimeZone() throws JspTagException {
        try {
            return (TimeZone) GET_TIME_ZONE_METHOD.invoke(null,
    ...
    public void setType(String type) {
        this.type = StringUtils.isBlank(type) ? null : Type.valueOf(type
                .toUpperCase());
    }

    public void setDateStyle(String style) {
        this.dateStyle = StringUtils.isBlank(style) ? null : Style
                .valueOf(style.toUpperCase());
    }

    public void setTimeStyle(String style) {
        this.timeStyle = StringUtils.isBlank(style) ? null : Style
                .valueOf(style.toUpperCase());
    }
